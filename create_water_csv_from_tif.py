...
import add_time_to_niwa

_DATA_ROOT = Path(r"./data")
# Path to the file of point locations to query.
GAUGE_POINTS_PATH = _DATA_ROOT / "source_point.geojson"
# Path to the Flood model output .nc file
FLOOD_MODEL_OUTPUT_PATH = _DATA_ROOT / "Waimakariri_100y_42h_0c_Inundation_h_merged.tif"
ELEVATION_PATH = _DATA_ROOT / "elevation.tif"
# Path to the bounding box of the unreal level, may be smaller than the flood model output
UNREAL_LEVEL_BOUNDS_PATH = _DATA_ROOT / "Kaiapoi_1k.geojson"
...
def extract_depths_for_single_point(row: gpd.GeoSeries, model_output, elevation) -> gpd.GeoSeries:
    """
    ...
    """
    x = round(row.geometry.x)
    y = round(row.geometry.y)
    z = next(elevation.sample([(x, y)]))[0]
    for value in model_output.sample([(x, y)]):
        point_model_output = value
    times = model_output.count
    depths_for_times = {}
    for time_slice in range(times):
        depths_for_times[str(time_slice)] = point_model_output[time_slice]
    depths_for_times["geometry"] = Point(x, y, z)
    row.update(depths_for_times)
    return row


def extract_depths_for_points(gauge_points: gpd.GeoDataFrame, model_output, elevation) -> gpd.GeoDataFrame:
    """
    ...
    time_slices = model_output.count
    for time_slice in range(time_slices):
        point_depths[str(time_slice)] = pd.Series(dtype='int16')
    point_depths = point_depths.apply(lambda point: extract_depths_for_single_point(point, model_output, elevation),
                                      axis=1)
    point_depths.drop(['0'], axis=1, inplace=True)
    return point_depths
...
def main():
    ...
    with rasterio.open(FLOOD_MODEL_OUTPUT_PATH) as model_output, rasterio.open(ELEVATION_PATH) as elevation:
        gauge_depths = extract_depths_for_points(gauge_points, model_output, elevation)
    ...
    add_time_to_niwa.add_timestamps_to_csv(WATER_SOURCES_OUTPUT_PATH)
